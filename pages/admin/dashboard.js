...
export default function AdminDashboard() {
  const [metricas, setMetricas] = useState({
    ...
    rankingAutos: [],
    comisionesTerceros: [] // Nuevo estado para comisiones de terceros
  });
  ...
  const cargarMetricas = async () => {
    try {
      ...
      let comisionesAutosTerceros = {}; // Objeto para agrupar comisiones de terceros
      ...
      reservas.forEach((res) => {
        ...
        if (fechaInicio.getMonth() === hoy.getMonth() && fechaInicio.getFullYear() === hoy.getFullYear()) {
          ...
          // Si es un vehículo de tercero, agrupamos la comisión generada por vehículo
      ...
      // Convertir comisiones de terceros a un array ordenado
      ...
      setMetricas({
        ...
        comisionesTerceros: comisionesTercerosArray
      });
    } catch (error) {
      ...
      <div style={{ padding: '25px' }}>
        ...
        {cargando ? (
          ...
          <>
            ...
            {/* NUEVA SECCIÓN: CUADRO DE COMISIONES Y DETALLES DE VEHÍCULOS DE TERCEROS */}
            <div style={{ backgroundColor: '#111', padding: '20px', borderRadius: '6px', border: '1px solid #222', marginBottom: '25px' }}>
              <h2 style={{ fontSize: '15px', color: '#d4af37', marginTop: 0, marginBottom: '15px' }}>💼 Comisiones de Vehículos de Terceros (Mes)</h2>
              {metricas.comisionesTerceros.length === 0 ? (
                <p style={{ color: '#666', fontSize: '13px', margin: 0 }}>No se han generado comisiones por vehículos de terceros en este mes.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {metricas.comisionesTerceros.map((item, index) => (
                    <div key={index} style={{ backgroundColor: '#181818', padding: '12px 15px', borderRadius: '4px', border: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px' }}>
                      <div>
                        <span style={{ color: '#d4af37', fontWeight: 'bold', marginRight: '10px' }}>🚗</span>
                        <strong style={{ color: '#fff' }}>{item.nombre}</strong>
                        <span style={{ color: '#888', marginLeft: '10px' }}>({item.veces} {item.veces === 1 ? 'renta generada' : 'rentas generadas'})</span>
                      </div>
                      <div style={{ color: '#22c55e', fontWeight: 'bold' }}>
                        Comisión: USD ${item.comisionTotal.toLocaleString()}
                      </div>
                    </div>
                  ))}
                </div>
