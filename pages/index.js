...
export default function Home() {
  ...
  const handleSubmitReserva = async (e) => {
    ...
    try {
      ...
      // 2. Intentar enviar correo con EmailJS (capturando errores individualmente)
      try {
        await emailjs.send(
        ...
        alert('Reserva guardada en el sistema, pero hubo un fallo al enviar el correo automático. Contacta por WhatsApp.');
      }

      setVehiculoSeleccionado(null);
    } catch (error) {
      console.error('Error general al guardar:', error);
      alert('Error crítico al guardar la reserva: ' + error.message);
    } finally {
      setEnviando(false);
    }
  };
